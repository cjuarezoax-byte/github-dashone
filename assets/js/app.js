...
function tickClock(){ const el=$('#clock'); if(el) el.textContent=new Date().toLocaleString(undefined,{dateStyle:'medium',timeStyle:'short'}); }
...
$('#themeToggle')?.addEventListener('click',()=>{ const next=document.documentElement.getAttribute('data-theme')==='dark'?'light':'dark'; document.documentElement.setAttribute('data-theme',next); storage.set('theme',next); pushActivity('Tema cambiado a '+next); });
const linksKey='dashone.links';
function renderLinks(){ const links=storage.get(linksKey,[{label:'Gmail',url:'https://mail.google.com'},{label:'Calendar',url:'https://calendar.google.com'},{label:'GitHub',url:'https://github.com'}]); const ul=$('#links'); if(!ul) return; ul.innerHTML=''; for(const link of links){ const li=document.createElement('li'); const a=document.createElement('a'); a.href=link.url; a.target='_blank'; a.textContent=link.label; const rm=document.createElement('button'); rm.className='btn ghost'; rm.textContent='✖'; rm.addEventListener('click',()=>{ storage.set(linksKey,storage.get(linksKey,[]).filter(x=>x.url!==link.url)); renderLinks(); pushActivity('Eliminaste acceso: '+link.label); }); li.append(a,rm); ul.append(li);} storage.set(linksKey,links); }
$('#addLink')?.addEventListener('click',()=>{ const label=prompt('Nombre del acceso'); if(!label) return; const url=prompt('URL (https://...)'); if(!url) return; const links=storage.get(linksKey,[]); links.push({label,url}); storage.set(linksKey,links); renderLinks(); pushActivity('Agregaste acceso: '+label); });
...
$('#globalSearch')?.addEventListener('input',e=>{ const q=e.target.value.toLowerCase(); const hits=[]; storage.get(todoKey,[]).forEach(t=>{ if(t.text.toLowerCase().includes(q)) hits.push('Tarea: '+t.text); }); if(storage.get(notesKey,'').toLowerCase().includes(q)) hits.push('Notas: coincidencia'); if(q.length>1) pushActivity(`Búsqueda: "${q}" → ${hits.length} resultado(s)`); });
function pushActivity(text){ const ul=$('#activity'); if(!ul) return; const li=document.createElement('li'); const left=document.createElement('div'); left.textContent=text; const time=document.createElement('small'); time.className='muted'; time.textContent=new Date().toLocaleTimeString(); li.append(left,time); ul.prepend(li); }
function init(){ renderLinks(); renderTodos(); const year=$('#year'); if(year) year.textContent=new Date().getFullYear(); pushActivity('Bienvenido a DashOne ✨'); }
